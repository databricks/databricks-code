...
class TestMcpProxyUnsupportedSdk:
    """`ucode mcp-proxy` must surface an incompatible `mcp` SDK as an actionable
    reinstall message, not a raw ImportError (issue #307)."""

    def test_message_names_installed_version_and_supported_range(self):
        from ucode.cli import _unsupported_mcp_sdk_message

        msg = _unsupported_mcp_sdk_message(
            ImportError("cannot import name 'streamablehttp_client'")
        )

        assert "mcp>=1.28.0,<2" in msg
        assert "streamablehttp_client" in msg
        # The documented workaround from the issue.
        assert '--with "mcp<2"' in msg

    def test_import_failure_exits_cleanly_with_guidance(self, monkeypatch):
        # Simulate mcp 2.x: importing the proxy raises ImportError. The command
        # must translate it to a non-zero exit with the reinstall hint, rather
        # than letting the traceback surface as an opaque connection failure.
        import builtins

        real_import = builtins.__import__

        def fake_import(name, *args, **kwargs):
            if name == "ucode.mcp_proxy":
                raise ImportError("cannot import name 'streamablehttp_client'")
        ...
        result = CliRunner().invoke(
            ...
            [
                "mcp-proxy",
                "--url",
                "https://x/api/2.0/mcp/functions/system/ai",
                "--host",
                "https://x",
                "--profile",
                "p",
        ...
        assert result.exit_code == 1
        assert "mcp>=1.28.0,<2" in _strip_ansi(result.output)
