...
def render_codex_config(workspace: str, use_ai_gateway_v2: bool, org_id: str | None) -> str:
    ...
    return (
        "# Managed by dbx-code. Run `dbx-code logout` to restore the prior config.\n"
        'profile = "default"\n'
        "\n"
        "[profiles.default]\n"
...
def configure_model_for_tool(tool: str, model: str | None) -> int:
    ...
    print_note(f"Launch via `dbx-code --tool {tool}`.")
    return 0
...
def usage() -> int:
    ...
    workspace = state.get("workspace")
    if not workspace:
        raise RuntimeError("Workspace is not configured. Run `dbx-code configure` first.")
    if not bool(state.get("use_ai_gateway_v2")):
        raise RuntimeError(
            "Usage summary requires Databricks AI Gateway V2. "
            f"Run `dbx-code configure`, enable AI Gateway V2, then try again. Docs: {AI_GATEWAY_V2_DOCS_URL}"
        )
...
def status() -> int:
    ...
    print_note("Use `dbx-code configure` to update workspace settings or tool models.")
    print_note("Use `dbx-code logout` to clear managed configs and restore prior files.")
    return 0
...
def logout() -> int:
    ...
    print_success("dbx-code state cleared")
    return 0


def print_help() -> None:
    print(heading("dbx-code"))
    print(muted("Databricks-backed Codex, Claude Code, and Gemini bootstrap"))
    print()
    print(label("Commands"))
    print(f"  {value('dbx-code')} {muted('[--tool codex|claude|gemini] [--model MODEL] [tool-args...]')}")
    print("    Launch the selected tool using the saved workspace configuration.")
    print(f"  {value('dbx-code configure')}")
    print("    Interactively configure workspace settings or Claude/Gemini model selection.")
    print(f"  {value('dbx-code status')}")
    print("    Show the current workspace, tool configs, and saved model selections.")
    print(f"  {value('dbx-code usage')}")
    print("    Show a fixed Databricks AI Gateway usage summary for the saved AI Gateway V2 workspace.")
    print(f"  {value('dbx-code logout')}")
    print("    Clear dbx-code state and restore any backed-up tool config files.")
    print()
    print(label("Behavior"))
    print_note("On first run, dbx-code prompts for your Databricks workspace settings.")
    print_note("`dbx-code configure` lets you choose whether to configure workspace settings or tool models.")
    print_note("Normal launches use the saved or explicit model and do not do interactive model discovery.")
    print_note("`dbx-code usage` fetches a fresh Databricks token each time, so expired one-hour tokens are handled automatically.")
    print_note("`dbx-code usage` shows a fixed last-7-days breakdown for Codex, Claude Code, and Gemini CLI.")
