...
The selected managed config (the workspace-published config normally, or the local setup draft when
``--local`` is passed) and the developer's own ucode state (``~/.ucode/state.json``) are never merged
on disk. Instead this module resolves them *per key* at config-write time: whatever the manifest
specifies wins, and anything it leaves unset falls back to
